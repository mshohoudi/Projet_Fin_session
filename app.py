...
def fig_comparaison_barres(res_ana, res_ef, res_ml, F, T):
    ...
    vm_vals = [
        ...
        res_ef["von_mises_max_Pa"] / 1e6,  # Correction de l'unité
...
def fig_scores_kfold(scores_cv):
    ...
    fig.update_layout(
        ...
        yaxis=dict(range=[0.8, 1.02], title="Score R² moyen"),
        paper_bgcolor="#ffffff", plot_bgcolor="#f8f9fa",
        font=dict(color="#212529"), height=300,
...
def fig_extrapolation_force(modele, cfg, T_fixe):
    ...
    for F in forces_N:
        ...
        # Résultat ML (avec les mêmes unités que le calcul standard)
...
# ════════════════════════════════════════════════════════════════
#  Métriques résumé : Retour à la version classique (3 colonnes)
# ════════════════════════════════════════════════════════════════

def afficher_metriques(res_ana, res_ef, res_ml):
    # Les unités sont maintenant corrigées et conformes à l'original
    ...
    vm_ef = res_ef["von_mises_max_Pa"] / 1e6  # Correction appliquée ici
